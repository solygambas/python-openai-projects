...
async function main() {
  ...
  for (const type of systemItemTypes) {
    ...
    if (existing) {
      ...
      finalType = await prisma.itemType.create({
        data: { name: type.name, icon: type.icon, color: type.color, isSystem: true, userId: null }
      });
    }
  ...
  const items = [
    ...
    // AI Workflows
    {
      ...
      content: 'Act as a senior software engineer. Review the following code for potential bugs, performance issues, and adherence to clean code principles. Suggest improvements where necessary.\n\nCode:\n[INSERT CODE HERE]',
      contentType: ContentType.TEXT,
      itemType: 'prompt',
    ...
    },
    {
      ...
      content: 'Based on the following TypeScript interface and function implementation, generate comprehensive TSDoc comments including @param, @returns, and @example sections.',
      contentType: ContentType.TEXT,
      itemType: 'prompt',
      collection: 'AI Workflows',
    },
    {
      ...
      content: 'Identify opportunities to use modern ES6+ features and functional programming patterns to make this code more concise and readable without changing its behavior.',
      contentType: ContentType.TEXT,
      itemType: 'prompt',
      collection: 'AI Workflows',
    },
