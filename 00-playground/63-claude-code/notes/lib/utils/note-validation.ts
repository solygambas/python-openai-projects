...
export function sanitizeAndValidateNote(rawTitle: unknown, rawContent: unknown): { success: true; value: SanitizedNote } | { success: false; error: any } {
  const sanitized = {
    title: typeof rawTitle === 'string' ? DOMPurify.sanitize(rawTitle, { ALLOWED_TAGS: [] }) : undefined,
    content: rawContent,
  }
  ...
  if (contentString.length > 50000) {
    return { success: false, error: new Error('Content cannot exceed 50000 characters') }
  }
