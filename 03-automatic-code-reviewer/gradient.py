...
def gradient_between_colors(
    color1: ColorRGB, color2: ColorRGB, steps: int
) -> List[ColorRGB]:
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "color1",
        type=str,
        help='First RGB color in the format "R,G,B" (e.g. "255,0,0")',
    )
    parser.add_argument(
        "color2",
        type=str,
        help='Second RGB color in the format "R,G,B" (e.g. "0,0,255")',
    )
    parser.add_argument(
        "-n", type=int, default=10, help="Number of gradient steps (default: 10)"
    )
